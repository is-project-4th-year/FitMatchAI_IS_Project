# FitMatch AI — Goal-Driven Workout Planner & Weekly Adaptation (Android + Firebase + Cloud Run)
...
- **Goal Management**
  ...
  - After saving a goal: **snackbar** prompt “Open Plan” → navigates to Plan screen.
...
- **Workout Logging & Weekly Finalization**
  ...
  - Tap **Finalize This Week** to compute adherence:
    - `completion_pct` (≥ 0.40 gate), `volume_scale`, `intensity_scale`, `progression_bias` (−1/0/1).
...
- **UX Improvements (Supervisor Feedback)**
  ...
  - **Snackbars** for success (“Workout plan created”, “New plan generated”).
  - Clear calls-to-action: after goal creation, suggest **Create your plan**.

- **Robust Sync**
  - **Snapshot flip-back guard**: short “hold window” + “sticky latest” to prevent UI reverting to stale Firestore snapshots.
...
├─ screens/
│  ├─ GoalScreen.kt         # Create/manage goals + snackbar (“Open Plan”)
│  ├─ PlanScreen.kt         # Tabs: Current Plan / Custom Workout
│  └─ ProgressScreen.kt     # Charts + weekly summaries
├─ Viewmodels/
│  ├─ GoalsViewModel.kt     # goalsFlow, CRUD, progress updates
│  └─ PlanViewModel.kt      # observePlans, generate, finalize+regen, guards
├─ models/
│  ├─ GoalsRepository.kt    # Firestore (users/{uid}/goals)
│  └─ PlanRepositoryImpl.kt # Retrofit (predict), Firestore (workoutplan, features)
├─ data/
│  ├─ Goal.kt               # progress computed field
│  ├─ PlanDTO.kt, ExerciseDTO.kt
│  └─ AdherenceSummary.kt
└─ net/
└─ FitmatchApi.kt        # /predict (FastAPI on Cloud Run)

```

---

## 🔌 Firestore Collections (by user)
...
│  └─ {goalId} {
│       goalType: "hypertrophy" | "fatloss" | "endurance",
│       durationWeeks: Int,
│       workoutsPerWeek: Int,
│       startDate: Long (ms),
│       endDate:   Long (ms),
...
│     }
├─ features/                      # Latest base features snapshot
│  └─ auto doc {
│       age, height, weight, bmi, goal_type, workouts_per_week, calories_avg, equipment,
│       submitted_at: serverTimestamp()
│     }
├─ workoutplan/                   # Saved plans
│  └─ auto doc {
│       plan_id, microcycle_days, exercises[], notes, model_version,
│       created_at: serverTimestamp(),
│       created_at_ms: Long (client now),
│       week_index: Int,
│       day_offset: Int
│     }
├─ plan_logs/{planId}/days/       # Per-day logs for adherence
│  └─ {dayDoc} {
│       date: Long (ms),
│       exercises_count: Int,
│       exercises_completed: Int,
│       volume_ratio: Double,
│       intensity_ratio: Double
│     }
└─ adherence_summaries/
└─ {planId}_{YYYY-Www} {
...
1. **Create Your Goal**
   ...
   * Save → snackbar suggests **Open Plan**.

2. **Generate Plan**

   * Go to **Plan → Custom Workout**, check/prefill your metrics, tap **Save Workout Plan**.
   * Snackbar: “**Workout plan created**”; tab snaps to **Current Plan**.
...
4. **Finalize Week**
   ...
   * Tap **Finalize This Week** → computes adherence → if pass, **new week** is generated:

     * Day numbering continues (e.g., 1-3 → 4-6).
...
* **Phone Auth error (17028 / Invalid app info)**
  Add **package**, **SHA-1**, **SHA-256** in Firebase → Project settings → Apps.
  Re-download `google-services.json`.

* **Plan “reverts” to old one after creation**
...
* **Days reset to 1 each week**
  The repository writes **`day_offset`** and shifts exercise `day` → UI groups by **absolute day**.
